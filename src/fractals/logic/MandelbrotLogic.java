...
public class MandelbrotLogic implements FractalLogic {
	...
	private final CoordinatesLogic coordinatesLogic;

	public MandelbrotLogic(int width, int height) {
		coordinatesLogic = new CoordinatesLogic(width, height, Complex.ofCannonical(-2, 2),
				Complex.ofCannonical(1, -2));

		// colorizer = new BufferedColorizer(new LinearColorizer(110,
		// Color.BLUE, Color.CYAN, Color.YELLOW, Color.RED, Color.GREEN));
		// colorizer = new LinearColorizer(100, Color.MAGENTA, Color.WHITE,
		// Color.RED, Color.BLUE, Color.WHITE);
		colorizer = new BufferedColorizer(new LinearColorizer(50, Color.RED, Color.MAGENTA, Color.PINK, Color.RED,
	...
	private void computeNumberOfIters() {
		final double minDiameter = coordinatesLogic.minDiameter();
	...
	public Color getColorAt(Complex c) {
		...
		final SequenceValue value = getValue(c, numberOfIters);
	...
	public Image generateImage() {
		...
		log.info("Generating image of " + coordinatesLogic.getTopLeft() + " to " + coordinatesLogic.getBottomRight()
				+ "\nNumber of iters is " + numberOfIters);
		final long startTime = System.currentTimeMillis();

		final BufferedImage im = new BufferedImage(coordinatesLogic.getWidth(), coordinatesLogic.getHeight(),
